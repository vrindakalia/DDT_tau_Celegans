...
adults <- all_ever %>% 
    ...
    scale_x_discrete(labels = c("N2 \n(Control)", "BR5271 \n(Non-agg)", "BR5270 \n(Agg)")) +
    ggtitle("70-72 hours: adult stage")
...
l4s.tof <- all_ever %>% 
    ...
    guides(shape = guide_legend(override.aes = list(size = 0.7))) +
    guides(fill = guide_legend(override.aes = list(size = 0.5))) +
    theme(legend.text = element_text(size = 7), axis.text.x = element_text(angle = 30, hjust=1,size = 7),
          plot.title = element_text(size = 10)) +
    ...
    scale_x_discrete(labels = c("N2 (Control)", "BR5271 (Non-agg)", "BR5270 (Agg)")) +
    ggtitle("46-50 hours") +
    ylim(0,2590)
